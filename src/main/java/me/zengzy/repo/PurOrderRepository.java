...
public interface PurOrderRepository extends CrudRepository<PurOrders, Long> {
    ...
    @Query(value = "SELECT * FROM pur_orders WHERE purchaser_name = :purchaser_name AND FIND_IN_SET(order_status, :statusSet) ORDER BY pur_serial_no LIMIT :cur, :pgSize", nativeQuery = true)
